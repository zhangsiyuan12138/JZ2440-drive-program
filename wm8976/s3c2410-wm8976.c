...
{
  	uda1341_volume = 57;
	uda1341_boost = 0;  //声音默认值
    wm8976_write_reg(0,0);  //复位
...
static int s3c2410iis_probe(struct device *dev) 
{
	...
	clk_enable(iis_clock);               //使能IIS控制器时钟

	local_irq_save(flags);
	
	/* GPB 4: L3CLOCK, OUTPUT */
	s3c2410_gpio_cfgpin(S3C2410_GPB4, S3C2410_GPB4_OUTP);          // L3CLOCK
	s3c2410_gpio_pullup(S3C2410_GPB4,1);
	/* GPB 3: L3DATA, OUTPUT */
	s3c2410_gpio_cfgpin(S3C2410_GPB3,S3C2410_GPB3_OUTP);           // L3DATA
	/* GPB 2: L3MODE, OUTPUT */
	s3c2410_gpio_cfgpin(S3C2410_GPB2,S3C2410_GPB2_OUTP);            // L3MODE
	s3c2410_gpio_pullup(S3C2410_GPB2,1);
	/* GPE 3: I2SSDI */ 
	s3c2410_gpio_cfgpin(S3C2410_GPE3,S3C2410_GPE3_I2SSDI);         //  I2SSDI
	s3c2410_gpio_pullup(S3C2410_GPE3,0);
	/* GPE 0: I2SLRCK */
	s3c2410_gpio_cfgpin(S3C2410_GPE0,S3C2410_GPE0_I2SLRCK);         //  I2SLRCK
	s3c2410_gpio_pullup(S3C2410_GPE0,0);
	/* GPE 1: I2SSCLK */
	s3c2410_gpio_cfgpin(S3C2410_GPE1,S3C2410_GPE1_I2SSCLK);          //  I2SSCLK   IIS数据传输时钟
	s3c2410_gpio_pullup(S3C2410_GPE1,0);
	/* GPE 2: CDCLK */
	s3c2410_gpio_cfgpin(S3C2410_GPE2,S3C2410_GPE2_CDCLK);            //   CDCLK    wm8976芯片工作时钟
	s3c2410_gpio_pullup(S3C2410_GPE2,0);
	/* GPE 4: I2SSDO */
	s3c2410_gpio_cfgpin(S3C2410_GPE4,S3C2410_GPE4_I2SSDO);           //  I2SSDO
	s3c2410_gpio_pullup(S3C2410_GPE4,0);

	local_irq_restore(flags);

	init_s3c2410_iis_bus();              //设置IIS控制器

	init_wm8976();         //使用L3接口初始化 wm8976 芯片

	output_stream.dma_ch = DMACH_I2S_OUT;                   // 设置DMA通道，用于播放
	...
	input_stream.dma_ch = DMACH_I2S_IN;                    // 设置DMA通道，用于录音
	...
	audio_dev_dsp = register_sound_dsp(&smdk2410_audio_fops, -1);    //注册字符设备 dsp       /dev/dsp  播音/录音
	audio_dev_mixer = register_sound_mixer(&smdk2410_mixer_fops, -1);   //注册字符设备 mixer  /dev/mixer         音量 
