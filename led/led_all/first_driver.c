...
{
   major = register_chrdev(0,"first_drvice",&first_drvice_mode);    // 可以通过 /proc/first_drvice
   ...
   firstdrv_class_dev = class_device_create(firstdrv_class,NULL,MKDEV(major,0),NULL,"xyz");  //在 /sys/class/firstdrv 下生成 xyz/设备目录
                                                                                              //并调用mdev   ,生成设备节点  , /dev/first_drivce
   gpfcon = (volatile unsigned long*)ioremap(0x56000050,16);       //用户空间一段地址关联到设备内存上
   gpfdat = gpfcon +1;
